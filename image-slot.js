...
(() => {
  ...
  function load() {
    ...
    // This page may live in a subfolder while the sidecar is written to the
    // project root (the host forces *.state.json writes to root). A plain
    // relative fetch would 404, so walk up parent dirs until it resolves.
    const __candidates = [STATE_FILE, '../' + STATE_FILE, '../../' + STATE_FILE, '../../../' + STATE_FILE];
    const __findSidecar = async () => {
      for (const __p of __candidates) {
        try { const __r = await fetch(__p); if (__r.ok) return await __r.json(); } catch (e) {}
      }
      return null;
    };
    loadP = __findSidecar()
